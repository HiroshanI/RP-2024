...
# RANDOM INSERTION ---------------------------------------------------
def add_word(new_words, wv_from_text):
    synonyms = []
    counter = 0
    ...
    while len(synonyms) <1:
        ...
        synonyms = list(get_synonyms_vec(random_word, wv_from_text))
        counter += 1
        if counter > 10:
            return
...
def random_insertion(words, n, wv_from_text):
    new_words = words.copy()
    for _ in range(n):
        add_word(new_words, wv_from_text)
    return new_words
...
def get_synonyms_vec(word, wv_from_text):
    synonyms = set()
    flag = False
    vec = None
    try:
...
def synonym_replacement_vec(words, n, wv_from_text, stop_words):
    new_words = words.copy()
    random_word_list = list(set([word for word in words if word not in stop_words]))
    random.shuffle(random_word_list)
    num_replaced = 0
    for random_word in random_word_list:
        synonyms = get_synonyms_vec(random_word, wv_from_text)
        if len(synonyms) >= 1:
    ...
    def augmentation(self, sentence, alpha_sr=0.1, alpha_ri=0.1, alpha_rs=0.1, alpha_rd=0.1, num_aug=9):
        ...
        if (alpha_sr > 0):
            ...
            for _ in range(num_new_per_technique):
                a_words = synonym_replacement_vec(words, n_sr, self.wv_from_text, self.stop_words)
                augmented_sentences.append(' '.join(a_words))
        #random insertion
        if (alpha_ri > 0):
            n_ri = max(1,int(alpha_ri * num_words))
            for _ in range(num_new_per_technique):
                a_words = random_insertion(words, n_ri, self.wv_from_text)
                augmented_sentences.append(' '.join(a_words))
        #Random Deletion
        if (alpha_rd > 0):
            for _ in range(num_new_per_technique):
